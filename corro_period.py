"""
CORRO PERIOD REPORT — v3.3-strict-discount-zero-discount-source
==============================
...
def _clean_join(values, sep=" | "):
    """Join non-empty unique strings while preserving order."""
    out = []
    seen = set()
    for v in values or []:
        txt = str(v or "").strip()
...
def order_discount_code_text(order):
    """Human-readable order-level discount codes/titles.

    This captures discount_codes and discount_applications so audit rows can show
    values such as 'Gift card for 100 dollar' when Shopify exposes it as a code,
    title, description, or application field.
    """
    values = []

    for dc in (order.get("discount_codes") or []):
        if isinstance(dc, dict):
            values.extend([dc.get("code"), dc.get("type")])
        else:
            values.append(dc)

    for app in (order.get("discount_applications") or []):
    ...
    return _clean_join(values)

def discount_application_label(app):
    """Readable label for one Shopify discount application."""
    if not isinstance(app, dict):
        return ""
    main = _clean_join([app.get("code"), app.get("title"), app.get("description")], sep=" / ")
    meta = _clean_join([app.get("type"), app.get("target_type"), app.get("target_selection"), app.get("allocation_method")], sep=", ")
    val = str(app.get("value") or "").strip()
    pieces = []
    if main:
        pieces.append(main)
    if val:
        pieces.append(f"value={val}")
    if meta:
        pieces.append(f"({meta})")
    return " ".join(pieces).strip()

def line_discount_origin(order, line_item):
    """Explain exactly where the line discount came from.

    Uses discount_allocations.discount_application_index to point back to
    order.discount_applications. Falls back to order discount codes/titles.
    """
    apps = order.get("discount_applications") or []
    origins = []

    for alloc in (line_item.get("discount_allocations") or []):
        idx = alloc.get("discount_application_index")
        amount = money(alloc.get("amount"))
        label = ""
        if isinstance(idx, int) and 0 <= idx < len(apps):
            label = discount_application_label(apps[idx])
        if not label:
            label = order_discount_code_text(order)
        if label and amount:
            origins.append(f"{label} — ${amount:.2f}")
        elif label:
            origins.append(label)

    if not origins:
        origins.append(order_discount_code_text(order))

    return _clean_join(origins)

def order_payment_gateway_text(order):
    gateways = order.get("payment_gateway_names") or []
    return _clean_join(gateways)

def is_staff_tag(tags_str):
    t = (tags_str or "").lower()
    return any(k in t for k in ["staff", "internal", "employee"])

# Orders/lines with replacement or refund context must not be counted as
# marketing/free-item 100% discounts. They remain in normal sales/staff/cost
# audits; this exclusion only affects q_discount_zero and q_discount_zero_detail.
DISC_ZERO_EXCLUDE_KEYWORDS = [
    "replacement", "replace", "replaced", "re-ship", "reship", "reshipment",
    "refund", "refunded", "return", "returned", "exchange",
]
...
def discount_zero_exclusion_reason(order, line_item, order_discount_codes=""):
    """Return why a strict 100% discount should be excluded from Discount Zero.

    Discount Zero is for intentional free/marketing/internal items. Replacement,
    refund, return, and exchange lines can also appear with 100% discounts in
    Shopify, but they should not inflate the 100% Discount report.
    """
    financial_status = (order.get("financial_status") or "").lower()
    if "refund" in financial_status or "void" in financial_status:
        return f"financial_status={financial_status}"

    order_text = " | ".join(str(v or "") for v in [
        order.get("tags"),
        order.get("source_name"),
        order.get("name"),
        order_discount_codes,
    ])
    if _contains_any_keyword(order_text, DISC_ZERO_EXCLUDE_KEYWORDS):
        return "replacement/refund keyword on order"

    line_text = " | ".join(str(v or "") for v in [
        line_item.get("title"),
        line_item.get("name"),
        line_item.get("sku"),
        line_item.get("vendor"),
    ])
    if _contains_any_keyword(line_text, DISC_ZERO_EXCLUDE_KEYWORDS):
        return "replacement/refund keyword on line item"
...
def shopify_graphql(query, variables=None):
    ...
    for attempt in range(8):
        ...
        if r.status_code in (500,502,503,504): time.sleep(min(2**attempt,90)); continue
        r.raise_for_status()
        d = r.json()
...
def _first_shopifyql_row(rows):
    """Return the first non-summary data row from ShopifyQL, normalized."""
    if not rows:
        return {}
    for row in rows:
        if isinstance(row, dict):
            # No GROUP BY summary queries usually return one row.
            return row
    return {}


def _summary_from_row(row):
    """Normalize direct ShopifyQL summary row to dashboard metric names.

    Important: Shopify dashboard summary must come from a non-grouped ShopifyQL
    query, not from summed product rows. Summing product rows can miss report-level
    adjustments/reversals and causes Q1/month differences.
    """
    return {
        "gross_sales": row_float(row, "gross_sales", "Gross sales"),
        "discounts": row_float(row, "discounts", "Discounts"),
        "sales_reversals": row_float(row, "sales_reversals", "Sales reversals", "returns", "Returns"),
        "net_sales": row_float(row, "net_sales", "Net sales"),
        "cogs": row_float(row, "cost_of_goods_sold", "Cost of goods sold", "cogs", "COGS"),
        "gross_profit": row_float(row, "gross_profit", "Gross profit"),
        "units": row_int(row, "net_items_sold", "Net items sold"),
        "orders": row_int(row, "orders", "Orders"),
    }
...
def fetch_shopifyql_summary_single(start, end):
    """Fetch one exact report-level Summary directly from ShopifyQL.

    Tries sales_reversals first because it matches Shopify Analytics Summary better.
    Falls back to returns if a store/schema does not expose sales_reversals.
    """
    base = (
        "FROM sales "
        "SHOW gross_sales, discounts, {reversal_metric}, net_sales, "
        "cost_of_goods_sold, net_items_sold, orders, gross_profit "
        f"SINCE {start} UNTIL {end}"
    )

    for reversal_metric in ("sales_reversals", "returns"):
        q = base.format(reversal_metric=reversal_metric)
        rows = shopifyql_table(q, f"summary metrics ({reversal_metric})")
        if rows:
            out = _summary_from_row(_first_shopifyql_row(rows))
            out["reversal_metric"] = reversal_metric
            return out

    print("  ⚠ ShopifyQL direct summary unavailable — returning zeros")
    return {
        "gross_sales": 0.0, "discounts": 0.0, "sales_reversals": 0.0,
        "net_sales": 0.0, "cogs": 0.0, "gross_profit": 0.0,
        "units": 0, "orders": 0, "reversal_metric": "",
    }


def fetch_shopifyql_summary(start, end, months):
    """Fetch total + per-month Summary from ShopifyQL, without GROUP BY product."""
    print("  Fetching ShopifyQL summary metrics...")
    total = fetch_shopifyql_summary_single(start, end)
    monthly = {}
    for m in months:
        ms = date(start.year, m, 1)
        me = date(start.year, m, calendar.monthrange(start.year, m)[1])
        # Clamp monthly range to selected period boundaries.
        ms = max(ms, start)
        me = min(me, end)
        if ms > me:
            monthly[m] = {
                "gross_sales": 0.0, "discounts": 0.0, "sales_reversals": 0.0,
                "net_sales": 0.0, "cogs": 0.0, "gross_profit": 0.0,
                "units": 0, "orders": 0, "reversal_metric": total.get("reversal_metric", ""),
            }
        else:
            monthly[m] = fetch_shopifyql_summary_single(ms, me)

    print("  ✓ SUMMARY SOURCE: ShopifyQL direct totals")
...
def aggregate(orders, vmap, months, order_staff_map=None, shopifyql_staff_order_map=None):
    ...
    for order in orders:
        ...
        order_discount_codes = order_discount_code_text(order)
        order_payment_gateways = order_payment_gateway_text(order)
        ...
        for li in line_items:
            ...
            # Discount handling
            # -----------------
            # For normal sales aggregation, we keep the best available line discount.
            # For the 100% Discount tab, however, we must be stricter: only rows whose
            # own Shopify line discount covers the full line gross are allowed.
            # We DO NOT classify original $0 items as 100% discounted, because there is
            # no gross price to prove a 100% discount.
            allocation_disc = sum(money(a.get("amount")) for a in (li.get("discount_allocations") or []))
            total_discount_disc = money(li.get("total_discount"))

            exact_line_disc = 0.0
            discount_source = ""
            if allocation_disc > 0:
                exact_line_disc = allocation_disc
                discount_source = "line.discount_allocations"
            elif total_discount_disc > 0:
                exact_line_disc = total_discount_disc
                discount_source = "line.total_discount"

            raw_line_disc = exact_line_disc
            if raw_line_disc <= 0 and order_discount_total > 0 and gross_line_total > 0:
                ...
                discount_source = "order.total_discounts_prorated"

            disc = min(raw_line_disc, gross) if gross > 0 else max(raw_line_disc, 0)
            net = max(gross - disc, 0)
            discount_pct = round((disc / gross), 6) if gross > 0 else 0.0

            # Strict 100% discount validation for q_discount_zero_detail.
            # Tolerance handles cent rounding only.
            pct_tolerance = 0.0001
            money_tolerance = 0.01
            exact_disc_for_validation = min(exact_line_disc, gross) if gross > 0 else 0.0
            exact_discount_pct = round((exact_disc_for_validation / gross), 6) if gross > 0 else 0.0
            is_exact_100_discount = (
                gross > money_tolerance
                and qty > 0
                and exact_line_disc > 0
                and exact_discount_pct >= (1.0 - pct_tolerance)
                and abs(gross - exact_disc_for_validation) <= money_tolerance
                and max(gross - exact_disc_for_validation, 0) <= money_tolerance
                and discount_source in ("line.discount_allocations", "line.total_discount")
            )
            ...
            base_audit_row = {
                ...
                "exact_discount_pct": exact_discount_pct,
                "discount_source": discount_source,
                "discount_code_source": discount_code_source,
                "order_discount_codes": order_discount_codes,
                "payment_gateways": order_payment_gateways,
                "discount_validation": discount_validation,
                "discount_zero_exclusion": discount_zero_exclusion,
                "net_paid": net,
            ...
            # 100% Discount tab: strict only.
            # Include only line items whose own Shopify line discount equals 100%.
            # Exclude original $0 items, order-level prorated fallback, and
            # replacement/refund/return/exchange context.
            if is_exact_100_discount and not discount_zero_exclusion:
...
def col_letter(n):
    """Convert 1-based column number to Google Sheets letters: 1=A, 26=Z, 27=AA."""
    result = ""
    while n:
        n, rem = divmod(n - 1, 26)
...
def write_tab(sh, name, headers, rows):
    """Write a whole tab without append_rows.

    Why:
    - append_rows can keep expanding the workbook and hit Google Sheets' 10M cell limit.
    - This function resizes the worksheet to the exact required size, clears old data,
      and writes the full range in one update.
    """
    needed_rows = max(len(rows) + 1, 50)
    ...
    # Make every row the same width as headers, so Sheets never tries writing past the range.
    width = len(headers)
    fixed_data = []
    for row in all_data:
        row = list(row)
    ...
    needed_rows = max(len(all_data), 50)
    needed_cols = max(len(headers), 1)

    # Resize down/up to the real size. This prevents cell-count growth across many periods.
    if ws.row_count != needed_rows or ws.col_count != needed_cols:
        sheets_call(ws.resize, rows=needed_rows, cols=needed_cols)
        time.sleep(0.5)

    # Clear previous values after resize.
    sheets_call(ws.clear)
    time.sleep(0.3)

    end_col = col_letter(needed_cols)
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  CORRO PERIOD REPORT {SCRIPT_BUILD} — {label}")
    ...
    # ── Totals ────────────────────────────────────────────────────
    # IMPORTANT:
    # Summary totals must match Shopify Analytics. Do NOT calculate Summary
    # by summing product rows or REST order rows. Product-level ShopifyQL
    # rows can differ from the report-level Summary because of reversals,
    # adjustments, grouping limits, refunds processed later, and rounding.
    total_gs    = summary_total.get("gross_sales", 0.0)
    total_disc  = summary_total.get("discounts", 0.0)
    total_ret   = summary_total.get("sales_reversals", 0.0)
    total_ns    = summary_total.get("net_sales", 0.0)
    total_cogs  = summary_total.get("cogs", 0.0)
    total_gp    = summary_total.get("gross_profit", 0.0)
    total_units = summary_total.get("units", 0)
    total_ords  = summary_total.get("orders", 0)
    gp_margin   = round(total_gp / total_ns * 100, 1) if total_ns else 0

    monthly_gs   = {m: summary_monthly.get(m, {}).get("gross_sales", 0.0) for m in months}
    monthly_disc = {m: summary_monthly.get(m, {}).get("discounts", 0.0) for m in months}
    monthly_ret  = {m: summary_monthly.get(m, {}).get("sales_reversals", 0.0) for m in months}
    monthly_ns   = {m: summary_monthly.get(m, {}).get("net_sales", 0.0) for m in months}
    monthly_cogs = {m: summary_monthly.get(m, {}).get("cogs", 0.0) for m in months}
    monthly_gp   = {m: summary_monthly.get(m, {}).get("gross_profit", 0.0) for m in months}
    monthly_u    = {m: summary_monthly.get(m, {}).get("units", 0) for m in months}
    ...
    # Guardrail: every row going to q_discount_zero_detail must be exactly 100%.
    invalid_disc_zero = [d for d in disc_zero_rows if d.get("discount_validation") != "EXACT_100_LINE_DISCOUNT" or abs(float(d.get("exact_discount_pct") or 0) - 1.0) > 0.0001]
    if invalid_disc_zero:
        raise RuntimeError(f"Discount Zero validation failed: {len(invalid_disc_zero)} non-100% rows detected")
    print(f"  ✓ Discount Zero validation: {len(disc_zero_rows)} rows are exact 100% line discounts, excluding replacement/refund context")
    ...
    for i, v in enumerate(sorted_vend):
        ...
        vend_rows.append([
            now, label, period_status, i+1, v["vendor"], len(v["skus"]), v["units"],
